...
import { IDailyQuizResponse, IQuizResultResponse } from '../../interfaces/quizInterface';
...
export const QuizSolving: React.FC = () => {
  ...
  const [quizResult, setQuizResult] = useState<IQuizResultResponse | null>(null);
  const size = useRecoilValue<EResize>(resizeState);

  //오늘의 퀴즈 api
  useEffect(() => {
    setLoading(true);
    api.get(`/${REQUEST_DOMAINS.quizs}/today`)
      .then(response => {
  ...
  const handleSubmitAnswer = () => {
    if (selectedOption && quizData) {
      api.post(`/${REQUEST_DOMAINS.quizs}/solve`, {
        ...
        <Modal>
          <div className={styles.container({ size })}>
            ...
            <img
              ...
              alt={quizResult.isCorrect ? "기뻐하는 페페" : "슬퍼하는 페페"}
